...
The **Custom** framework lets you describe an API convention with configuration rules when a project does not use one of EasyApi's built-in framework recognizers.

Unlike Spring, JAX-RS, Feign, and gRPC support, Custom has no hard-coded annotation detection. Class recognition, method recognition, HTTP method, path, and parameter binding are all controlled by `custom.*` rules. Once a rule produces a normal API endpoint, every enabled export channel can consume it.
...
See [Config Rules](/settings/config-rule) and [Rule Authoring](/settings/config-rule#ways-to-write-rules) for the rule file format and AI-assisted authoring workflow.
