...
EasyApi can export HTTP endpoints as a Hoppscotch collection. The channel supports both a local JSON file and upload to a Hoppscotch server.
...
2. Choose **EasyApi** > **Export** > **Hoppscotch (Beta)**.
...
When no token is configured, the generated JSON can be imported into Hoppscotch manually. With a token, EasyApi uploads the collection through the Hoppscotch API.
...
Updating a collection uses a create-first-then-delete flow. EasyApi asks for confirmation because the replacement collection receives a new ID. Shared links to the old collection therefore stop working after the update.
