...
EasyApi exports HTTP endpoints to an [ApiPost](https://www.apipost.cn/) project. Endpoints are pushed through ApiPost's open API when a token and a project are configured; otherwise an `apipost.json` import file is written so you can import it by hand.

## Enable the channel

Open **Settings** > **Other Settings** > **EasyApi** > **Features**. Under **Export Channels**, enable **ApiPost (Beta)**, then reopen Settings to reach the ApiPost tab — the same chicken-and-egg path as the other beta channels.

## Configure the target

Open **Settings** > **Other Settings** > **EasyApi** > **ApiPost (Beta)**:
...
2. Choose **EasyApi** > **Export** > **ApiPost (Beta)**, or use the ApiPost action from the API Explorer.
...
| `apipost.server.url` | The base URL of the API you are documenting. Written into the imported document's host/pre-URL (ApiPost's pre-URL), so imported endpoints point at your service. |
...
- [API Settings](/settings/ide-setting) — the ApiPost settings tab reference
