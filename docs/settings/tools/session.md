...
Session-scoped storage for scripts. Data stored in `session` is cleared when the IDE is restarted, making it ideal for temporary caching during a session. Aliases: `S`, `sessionStorage`
